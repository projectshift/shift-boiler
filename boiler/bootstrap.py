...
def add_debug_toolbar(app):
    ...
    from kernel.features.debug_toolbar import debug_toolbar_feature
    debug_toolbar_feature(app)

def add_routing(app):
    """ Add routing and lazy-views feature """
    from kernel.features.routing import routing_feature
    routing_feature(app)


def add_jinja_extensions(app):
    """ Activate custom jinja extensions """
    from kernel.features.jinja_extensions import jinja_extensions_feature
    jinja_extensions_feature(app)


def add_error_pages(app):
    """ Add custom error pages """
    from kernel.features.error_pages import error_pages_feature
    error_pages_feature(app)


def add_mail(app):
    """ Add mailing functionality """
    from kernel.features.mail import mail_feature
    mail_feature(app)


def add_orm(app):
    """ Add SQLAlchemy ORM integration """
    from kernel.features.orm import orm_feature
    orm_feature(app)


def add_navigation(app):
    """ Add navigation to app """
    from kernel.features.navigation import navigation_feature
    navigation_feature(app)


def add_api(app):
    """ Add restful api functionality """
    from kernel.features.api import api_feature
    api_feature(app)


def add_logging(app):
    """ Add logging functionality """
    from kernel.features.logging import logging_feature
    logging_feature(app)


def add_users(app):
    """ Add user management functionality """
    from kernel.features.users import users_feature
    users_feature(app)


def add_localization(app):
    """ Enable support for localization and translations"""
    from kernel.features.localization import localization_feature
